...
dat<-dat%>%mutate(parent_age=as.numeric(parent_age),
                  ...
                  children_count=case_when(number_children < 4 ~"1-3",
                                           number_children >= 4 ~"4+"),
                  parent_age_flag=case_when(parent_age <= 35 ~"35 and under",
...
final<-final%>%
  mutate(bulgaria_prop = bulgaria/table(dat$country)[1],
